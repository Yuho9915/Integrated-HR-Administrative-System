...
const mockUsers=[
  {username:'employee',password:'123456',role:'employee',name:'陈晓雨',department:'市场部',employeeNo:'EMP-1024',position:'招商主管'},
  {username:'manager',password:'123456',role:'manager',name:'王嘉铭',department:'市场部',employeeNo:'MGR-1001',position:'部门经理'},
  {username:'admin.hr',password:'123456',role:'hr',name:'于浩',department:'综合管理部',employeeNo:'HR-0001',position:'HRBP'},
  {username:'boss',password:'123456',role:'boss',name:'赵启明',department:'总经办',employeeNo:'BOSS-0001',position:'总经理'},
];
const init=()=>{const es=emp();return{employees:es,approvals:c(approvalRows),performance:perf(es),attendance:att(es),leaves:leaves(es),supplements:[],payroll:pay(es),administration:c(adminAssetsRows)}};
const read=()=>{try{return{...init(),...(JSON.parse(localStorage.getItem(K)||'null')||{})}}catch{return init()}};
const write=(s)=>(localStorage.setItem(K,JSON.stringify(s)),s);
const st=()=>write(read());

export const shouldHandleDevMock=(cfg={})=>on()&&[/^\/auth\/(login|logout|me|refresh)(?:\/|$)/,/^\/dashboard\/summary$/,/^\/reports\/summary$/,/^\/ai\/(health|chat|resume\/parse|id-card\/parse)$/,/^\/employees(?:\/|$)/,/^\/approvals(?:\/|$)/,/^\/performance(?:\/|$)/,/^\/payroll(?:\/|$)/,/^\/attendance(?:\/|$)/,/^\/leaves(?:\/|$)/,/^\/supplements(?:\/|$)/,/^\/administration\/(summary|assets)$/].some(r=>r.test(p(cfg.url)));
...
if(m==='post'&&u==='/auth/login'){const matched=mockUsers.find(item=>item.username===d.username&&item.password===d.password&&item.role===d.role);if(!matched){const error=new Error('用户名、密码或角色不正确');error.status=401;throw error;}return ok({token:`dev-token-${matched.role}`,refreshToken:`dev-refresh-${matched.role}`,user:{name:matched.name,role:matched.role,department:matched.department,employeeNo:matched.employeeNo,position:matched.position}},'登录成功');}
if(m==='post'&&u==='/auth/logout')return ok({},'已退出登录');
if(m==='post'&&u==='/auth/refresh')return ok({token:'dev-token',refreshToken:'dev-refresh-token'},'刷新成功');
if(m==='get'&&u==='/auth/me'){const employee=s.employees.find(i=>i.employee_no==='EMP-1024')||{};return ok({name:'陈晓雨',role:'employee',department:'市场部',employeeNo:'EMP-1024',position:'专员',hire_date:employee.hire_date||'2025-01-15',resignation_date:employee.resignation_date||'',political_status:employee.political_status||'群众',status:employee.status||'在职',phone:employee.phone||'13800001234',email:employee.email||'emp1024@yuhohr.local',gender:employee.gender||'女',birth_date:employee.birth_date||'1995-01-01',ethnicity:employee.ethnicity||'汉族',education:employee.education||'本科',graduate_school:employee.graduate_school||'示例大学',major:employee.major||'示例专业',job_level:employee.job_level||'P3',report_to:employee.report_to||'部门负责人',work_location:employee.work_location||'总部办公区',probation_end_date:employee.probation_end_date||'2025-04-15',contract_type:employee.contract_type||'固定期限劳动合同',contract_sign_date:employee.contract_sign_date||'2025-01-01',contract_end_date:employee.contract_end_date||'2027-12-31',current_address:employee.current_address||'示例居住地址',emergency_contact:employee.emergency_contact||'家属联系人',emergency_contact_phone:employee.emergency_contact_phone||'13900002000',id_card_attachments:employee.id_card_attachments||[],education_certificate_attachments:employee.education_certificate_attachments||[],labor_contract_attachments:employee.labor_contract_attachments||[],medical_report_attachments:employee.medical_report_attachments||[]});}
if(m==='get'&&u==='/dashboard/summary')return ok({cards:[{label:'总人数',value:18,tip:'在职员工',delta:'+2',trend:'up'},{label:'考勤异常',value:3,tip:'今日异常',delta:'-1',trend:'down'},{label:'待审批',value:5,tip:'待处理审批',delta:'+2',trend:'up'},{label:'本月实发',value:'183420',tip:'税后工资',delta:'+6.2%',trend:'up'}]});
if(m==='get'&&u==='/reports/summary')return ok({charts:[{name:'人力结构',value:'18人',tip:'正式员工 15 / 试用 3'},{name:'考勤表现',value:'96.2%',tip:'本月平均出勤率'},{name:'绩效达成',value:'87.4',tip:'全员绩效平均分'},{name:'行政支出',value:'¥26,800',tip:'本月行政与物料成本'}]});
if(m==='get'&&u==='/ai/health')return ok({status:'ok',provider:'dev-mock'});
if(m==='post'&&u==='/ai/chat')return ok({content:`AI 助手已收到问题：${d.prompt || '未提供内容'}。当前为开发模拟回复。`});
if(m==='post'&&u==='/ai/resume/parse')return ok({name:'候选人示例',education:'本科',skills:['沟通协调','数据分析']},'简历解析完成');
if(m==='post'&&u==='/ai/id-card/parse')return ok({name:'张三',idCardNo:'310101199001011234'},'身份证解析完成');
...
if(m==='post'&&/^\/approvals\/[^/]+\/decision$/.test(u)){write({...s,approvals:s.approvals.map(i=>i.id===u.split('/')[2]?{...i,status:d.decision}:i),supplements:(s.supplements||[]).map(i=>i.id===s.approvals.find(a=>a.id===u.split('/')[2])?.source_id?{...i,status:d.decision}:i)});return ok({},d.decision==='已通过'?'审批通过':'已驳回');}
...
if(m==='get'&&u==='/performance/summary')return ok({averageScore:87.4,pendingCount:s.performance.filter(i=>['待自评','待复核'].includes(i.status)).length,completedCount:s.performance.filter(i=>['已发布','已确认','已归档'].includes(i.status)).length});
...
if(m==='get'&&u==='/supplements')return ok(c(s.supplements||[]));
if(m==='post'&&u==='/supplements'){const created={id:gid('SUP'),leave_type:'补卡申请',start_at:`${d.date} ${d.time}`,end_at:`${d.date} ${d.time}`,days:0,status:'待审批',approver:'于浩',created_at:new Date().toISOString(),...d};write({...s,supplements:[created,...(s.supplements||[])],approvals:[{id:gid('APR'),category:'人事审批',applicant_employee_no:d.employee_no,applicant:d.employee_no,type:'补卡申请',duration:d.time,status:'待审批',level:'hr',assigned_to:'admin.hr',apply_time:new Date().toISOString(),reason:d.reason,related_info:[{label:'补卡日期',value:d.date},{label:'补卡时间',value:d.time}],history:[],source_id:created.id},...s.approvals]});return ok({supplement:created},'补卡申请已提交');}
if(m==='get'&&u==='/administration/summary')return ok({records:applyApprovedAssetSyncs(c(s.administration))});
if(m==='post'&&u==='/administration/assets'){const created={id:gid('AST'),...d};write({...s,administration:[created,...s.administration]});return ok({asset:created},'资产品类创建成功');}
